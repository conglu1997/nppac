...
def get_generic_path_information(paths, stat_prefix=''):
    ...
    # Attachment for computing binary success rates.
    for info_key in ['env_infos']:
        if info_key in paths[0]:
            ...
            if 'goal_achieved' in all_env_infos[0].keys():
                # Compute success rates
                bool_arr = [info['goal_achieved'] for info in all_env_infos]
                succ_rate = float([any(l) for l in bool_arr].count(True)) / float(len(bool_arr))
                statistics[stat_prefix + 'Goal Achieved Rate'] = succ_rate
